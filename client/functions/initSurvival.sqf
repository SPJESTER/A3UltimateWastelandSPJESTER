...
[] spawn  {
	while{true} do
	{
		...
		switch(true) do {
			...
			case (hungerLevel <= 80 && hungerLevel >= 75): {hint format["You haven't eaten anything in awhile, your hunger level is currently: %1\n\n You should find something to eat soon!", hungerLevel];};
			};
		};
	};
};
...
[] spawn  {
	...
	else
	{
		...
		switch(true) do {
			...
			case (thirstLevel <= 80 && thirstLevel >= 75): {hint format["You haven't drank anything in awhile, your thirst level is %1", thirstLevel];};
			};
		};
	};
};